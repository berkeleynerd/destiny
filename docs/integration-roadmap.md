...
| D-04 | Wire-format recovery: document the park-update stream (from `python/destiny/net` semantics and the C++ `WriteBallsToStream` path) and provide a scripted-scenario recorder | PL-33 | Accepted | D-01 | `docs/park-update-stream.md` is source-verified end to end (packet header, full mode-dependent ball record both directions, delta builder, and DoDestinyUpdate framing with the per-tick completion barrier). The wire cores live un-gated in `ParkStream.cpp`; `Destiny_WriteEmbeddedFullState` + `DestinyEmbeddedRecorderTest` pin the PL-11A scenario — which never enables shuffled proximity sensors — as byte-identical across two runs (19 packets, 3,328 bytes), with tick numbers and the scenario-scoped seed-absent marker journaled. |
| D-05 | Oracle role for kernel succession: replay-equivalence corpora (motion, collision predicates) and exported RNG state for journaling; destiny retires to reference implementation | PL-50 | Blocked | D-02, D-04 | Successor #1 is the Swift interim reference kernel (interim-reference doctrine, promised-land architecture 2026-07-14): it matches destiny bit-for-bit over the corpora on same-binary replay terms; the Safe/SPARK kernel then matches the Swift reference on the same terms; divergences adjudicated against the closed forms. |
| D-07 | Embedded warp command: `Destiny_CommandEmbeddedWarp` queues `Ballpark::WarpTo` through the accepted next-tick seam with warp-state diagnostics (effect stamp, factor, minimum range, total distance, collision/sensor participation) unpunned from the shanghaied ball members | PL-11C | Accepted | D-01, D-06 | `DestinyEmbeddedWarpContractTest`: preconditions reject wrong-tick, degenerate-factor, and sub-200 km (silent-downgrade-zone) commands; the 9.06 AU stargate-to-planet leg at warpFactor 5000 (Astero `warpSpeedMultiplier 5.0`, pl11a provenance) aligns in 8 ticks, activates at evolve 11, and drops out at evolve 31 below 100 m/s with collision and sensor participation restored; every warp-phase sample re-derives from the recovered closed forms within 20 ULP of the leg scale; the 42-sample corpus matches at `max(1e-5 m, 1e-12 rel)`; trajectories are bit-exact across ego/ego/observer runs and with the PL-12 celestial set present; scheduler-free Python events are counted as suppressed while the three warp events can be forwarded through the pre-advance host callback setter. |
...
| D-09 | Deterministic multi-ship embedded state: pre-advance dynamic STOP-ball addition, generic ball diagnostics, and all-ball packet-import history initialization | PL-C0 | Accepted | D-01, D-04 | `DestinyEmbeddedMultiBallContractTest` proves two unique free STOP balls, distinct live curves from frame zero, duplicate/cross-system/late rejection, direct-write → fresh-read → write byte fixed point, and matching first two evolves. Combat Rehearsal consumes the packet with the second ship as an ordinary ball rather than a celestial or navigation role. |
| D-10 | Native embedded missile lifecycle: next-tick late-ball launch, 800 ms straight-flight phase, homing, collision/expiry diagnostics, and terminal removal | PL-C1 | Accepted | D-09 | `DestinyEmbeddedMissileContractTest` proves owner 1 / target 2 `DSTBALL_MISSILE`, straight flight, homing, target collision before a 5,000 ms lifetime, expiry in the non-contact control, guarded removal, an unchanged initial two-ball packet, and byte-identical state samples/collision tick after replay. |
