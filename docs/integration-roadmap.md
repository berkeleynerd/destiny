...
| D-01 | Consumable-dependency contract: install with the consumer's exact triplet/toolchain (one carbon-blue binary across trees), and document the Blue module-registration entry a static consumer must invoke | PL-10 | Active | D-00 | Install tree consumed by `find_package(carbon-destiny)` from the trinity probe; registration steps documented here; probe links and registers destiny classes. |
